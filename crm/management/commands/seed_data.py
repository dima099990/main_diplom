...
запчасти, аксессуары, заказы, продажи, платежи, задачи, расходы.
"""
import random
...
from crm.models import (
    ...
    RepairOrderAccessory, RepairOrderPart, RepairOrderService,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        settings_obj.company_name = 'Kayros Service'
        settings_obj.tagline = 'Профессиональный ремонт телефонов'
        settings_obj.phone = '+7 (495) 123-45-67'
        settings_obj.address = 'г. Москва, ул. Примерная, д. 1'
        settings_obj.working_hours = 'Пн–Сб 10:00–20:00'
        settings_obj.warranty_days = 90
        settings_obj.about_text = 'Профессиональный сервисный центр по ремонту смартфонов.'
        settings_obj.about_full_text = (
        ...
        settings_obj.inn = '770123456789'
        settings_obj.ogrn = '321774600123456'
        ...
        settings_obj.bik = '044525225'
        settings_obj.account_number = '40802810938000123456'
        settings_obj.corr_account = '30101810400000000225'
        settings_obj.save()
        ...
        brand_objs = {}
        model_objs = {}
        for i, (brand_name, (icon, models)) in enumerate(brands_data.items()):
            brand, _ = Brand.objects.get_or_create(
                name=brand_name,
                defaults={'icon': icon, 'emoji': icon, 'order': i, 'is_active': True},
            )
            brand_objs[brand_name] = brand
            model_objs[brand_name] = []
            for j, m in enumerate(models):
                mo, _ = PhoneModel.objects.get_or_create(
                    brand=brand, name=m,
                    defaults={'order': j, 'is_active': True},
                )
                model_objs[brand_name].append(mo)

        # ── Услуги по ремонту ─────────────────────────────────────────────────
        services_data = [
            ('Замена дисплея (оригинал)', 12000, 18000, '1–2 часа', True),
            ('Замена дисплея (копия)',     4500,  8000,  '1–2 часа', False),
            ('Замена аккумулятора',        2500,  4500,  '30 мин',   True),
            ('Замена задней крышки',       1500,  3000,  '1–2 часа', False),
            ('Замена кнопки питания',      1200,  2500,  '30 мин',   False),
            ('Замена разъёма зарядки',     1800,  3500,  '1–2 часа', False),
            ('Диагностика',                500,   500,   '30 мин',   False),
            ('Чистка от влаги',            2000,  3500,  '1–2 дня',  False),
        ]
        for brand_name, models in model_objs.items():
            for model in models[:3]:
                for svc_name, price_from, price_to, duration, popular in services_data[:5]:
                    RepairService.objects.get_or_create(
                        phone_model=model, name=svc_name,
                        defaults={
                            'price_from': price_from + random.randint(-500, 2000),
                            'price_to': price_to,
                            'duration': duration,
                            'is_active': True,
                            'is_popular': popular,
        ...
        for name in ['ТехноЗапчасти', 'МобайлПарт', 'РемонтОпт', 'PhoneSpare']:
            s, _ = Supplier.objects.get_or_create(
                name=name, defaults={'phone': '+7 (800) 000-00-00', 'is_active': True},
            )
            suppliers.append(s)
        ...
        if not User.objects.filter(username='admin').exists():
            ...
            UserProfile.objects.update_or_create(
                user=admin,
                defaults={'role': 'admin', 'branch': branches[0]},
            )
        ...
        for username, fn, ln, role, rp, ap, branch in staff_data:
            if not User.objects.filter(username=username).exists():
                u = User.objects.create_user(
                    username, f'{username}@kayros.ru', 'pass123',
        ...
        parts_data = [
            ...
            ('Стекло Redmi Note 12',                 'Xiaomi',  'Redmi Note 12',      150,   500,   20, 5),
        ]
        part_objs = []
        ...
        for _ in range(40):
            ...
            # Услуга
            svcs = list(RepairService.objects.filter(phone_model=model, is_active=True))
            if svcs:
                svc = random.choice(svcs)
                RepairOrderService.objects.create(
                    order=order, service=svc, name=svc.name,
                    price=svc.price_from + Decimal(random.randint(0, 1000)),
                )
            ...
            if part_objs and random.random() > 0.4:
                ...
                if part.quantity >= qty:
                    RepairOrderPart.objects.create(
                        order=order, part=part, quantity=qty, price=part.sale_price,
                    )
        ...
        for _ in range(15):
            ...
            for acc in random.sample(acc_objs, min(random.randint(1, 3), len(acc_objs))):
        ...
        self.stdout.write(self.style.SUCCESS(
            f'\n✓ Готово!\n'
            f'  Заказов: {len(created_orders)} | Клиентов: {len(customers)}\n'
            f'  Запчастей: {len(part_objs)} | Аксессуаров: {len(acc_objs)}\n'
            f'\n  Логины:\n'
            f'  admin              / admin123\n'
            f'  master1–3          / pass123\n'
            f'  manager1–2         / pass123\n'
        ))
